...
KrakenVision::KrakenVision() {
	// AxisCamera &camera = AxisCamera::GetInstance("10.14.10.2");	//To use the Axis camera uncomment this line
	image = new RGBImage("/testImage.jpg");		// get the sample image from the cRIO flash
	_task = new Task("VisionTask",(FUNCPTR)KrakenVision::VisionTask);
	_task->Start((UINT32)this);
	KrakenVision::_enabled = false;
	KrakenVision::_timer = new Timer();
}
 ...
 */
double KrakenVision::computeDistance (BinaryImage *image, ParticleAnalysisReport *report) {
 ...
 */
double KrakenVision::scoreAspectRatio(BinaryImage *image, ParticleAnalysisReport *report, bool vertical){
 ...
 */
bool KrakenVision::scoreCompare(Scores scores, bool vertical){
	bool isTarget = true;
 ...
 */
double KrakenVision::scoreRectangularity(ParticleAnalysisReport *report){
 ...
 */
double KrakenVision::ratioToScore(double ratio)
{
	return (max((double)0, min(100*(1-fabs(1-ratio)), (double)100)));
}
 ...
 */
bool KrakenVision::hotOrNot(TargetReport target)
{
	bool isHot = true;
...
void KrakenVision::processImage()
{
	...
	if(reports->size() > 0)
	{
		...
		if(verticalTargetCount > 0)
		{
			...
			_distance = computeDistance(filteredImage, distanceReport);
			_hot = target.Hot;
			if(target.Hot)
			{
...
void KrakenVision::VisionTask(KrakenVision *vp)
{
	while(true)
	{
		if(vp->_enabled && (vp->_timer->Get() > (1.0/15.0)))
		{
			vp->processImage();
			vp->_timer->Reset();
		}
...
void KrakenVision::Start()
{
	//check to resume vision task, if not able to resume restart task, else return true (it resumed)
	_enabled = true;
	_timer->Start();
}
void KrakenVision::Stop()
{
	//checks to see if task is running, if yes suspend else return true (it's not running)
	_enabled = false;
	_timer->Stop();
}
